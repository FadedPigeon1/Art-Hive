  ...
  const exportTimelapse = useCallback(async () => {
    ...
    try {
      ...
      for (const ev of recordingEvents.current) {
        if (ev.type === 'stroke') {
           ctx.save();
           ctx.strokeStyle = ev.stroke.settings?.strokeStyle || ev.stroke.brushColor;
           ...
           if (ev.stroke.brushType === "AIRBRUSH" || ev.stroke.brushType === "CHARCOAL") {
               ctx.fillStyle = ctx.strokeStyle;
               ctx.beginPath();
               ctx.arc(ev.stroke.x, ev.stroke.y, ev.stroke.brushSize / 2, 0, Math.PI * 2);
               ctx.fill();
           ...
           if (ev.stroke.symmetryConfig && ev.stroke.symmetryConfig.mode !== 'none') {
              ...
              const symPts = [
                  { c: mirrorPoint({x: ev.stroke.x, y: ev.stroke.y}), l: mirrorPoint({x: ev.stroke.lastX, y: ev.stroke.lastY}) }
              ];

              if (type === 'radial') {
                  const mVert = (pt) => ({ x: axisX + (axisX - pt.x), y: pt.y });
                  const mHorz = (pt) => ({ x: pt.x, y: axisY + (axisY - pt.y) });
                  symPts.push({ c: mVert({x: ev.stroke.x, y: ev.stroke.y}), l: mVert({x: ev.stroke.lastX, y: ev.stroke.lastY}) });
                  symPts.push({ c: mHorz({x: ev.stroke.x, y: ev.stroke.y}), l: mHorz({x: ev.stroke.lastX, y: ev.stroke.lastY}) });
              }
      ...
      console.error(err);
      toast.error("Failed to export time-lapse. Your browser may not support WebM recording.");
      setIsExporting(false);
    }
