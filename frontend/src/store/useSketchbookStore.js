...
export const useSketchbookStore = create((set, get) => ({
  ...
  hsl: { h: 0, s: 100, l: 50 },
  setSwatches: (val) => set((state) => ({ swatches: typeof val === 'function' ? val(state.swatches) : val })),
  addSwatch: (color) => set((state) => ({ swatches: [...state.swatches, color] })),
  setHsl: (val) => set((state) => ({ hsl: typeof val === 'function' ? val(state.hsl) : val })),
  ...
  setLayers: (val) => set((state) => ({ layers: typeof val === 'function' ? val(state.layers) : val })),
  ...
  updateLayerOpacity: (id, opacity) => set((state) => ({
    layers: state.layers.map(l => l.id === id ? { ...l, opacity } : l)
  })),

  updateLayerBlendMode: (id, blendMode) => set((state) => ({
